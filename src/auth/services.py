...
from .models import DBUser, RegisterUserRequest, Token, User
...
async def register_user(
    ...
    Returns a dictionary containing a welcome message for the newly registered user.
    ...
    """
    # Check first if the username or email already exists in the database
    existing_user = (
    ...
    try:
        ...
        raise RegistrationFailed("Conflict detected during registration.")

    return {"detail": f"Welcome to BuklatAPI, {db_user.username}!"}


async def get_user_by_username(username: str, session: AsyncSession) -> DBUser | None:
    """Fetch a user from the database by their username.

    Returns a `DBUser` SQLAlchemy model contaiing the user's information if found; iotherwise, returns `None`.
    ...
    -------
    DBUser | None
        A SQLAlchemy model instance representing the user if found, otherwise `None`.
    """
    db_user = (
...
async def authenticate_user(
    ...
    """Authenticate a user by their verifying their username and password.

    Returns a `User` Pydantic model if authentication is successful; otherwise, returns `None`.
...
async def login_for_access_token(
    ...
    form_data : OAuth2PasswordRequestForm
        A FastAPI dependency that extracts `username` and `password` from a form. Injected via `Depends`.
