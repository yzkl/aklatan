...
) -> Callable[[Request, BuklatApiError], JSONResponse]:
    detail = {"message": initial_detail}

    async def exception_handler(_: Request, exc: BuklatApiError) -> JSONResponse:
        if exc.message:
...
app.add_exception_handler(
    ...
    handler=create_exception_handler(
        status.HTTP_401_UNAUTHORIZED, "Username or email has already been taken."
    ),
)
...
app.add_exception_handler(
    ...
    handler=create_exception_handler(
        ...
        "Account exists, but has been disabled or deactivated.",
    ),
)
...
app.add_exception_handler(
    ...
    handler=create_exception_handler(
        ...
        "A service seems to be down. Please try again later.",
    ),
)
